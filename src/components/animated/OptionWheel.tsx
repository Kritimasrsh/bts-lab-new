...
const OptionWheel = ({
  ...
  // Keep the latest props/config in refs. Assigned in an effect (not during
  // render) so React's rules-of-hooks immutability lint stays satisfied.
  useEffect(() => {
  ...
  // Keep a stable ref to the latest runFrame so the rAF loop can re-schedule
  // without referencing the callback before its declaration.
  useEffect(() => {
