...
export default function BrandPicker() {
  return (
    <motion.div
      ...
      className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-ink/10 bg-ink/10 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7"
    >
      {BRANDS.map((brand) => (
        <motion.div key={brand.slug} variants={card}>
          ...
          >
            <ArrowUpRight className="absolute right-2.5 top-2.5 h-4 w-4 text-brand opacity-0 transition-all duration-200 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:opacity-100" />
